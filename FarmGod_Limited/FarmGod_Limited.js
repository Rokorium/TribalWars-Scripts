...
window.FarmGod.Main = (function (Library, Translation) {
    ...
    // Add a global variable to track the last mouse movement time
    let lastMouseMoveTime = Date.now();

    // Add a mousemove event listener to update the lastMouseMoveTime
    document.addEventListener('mousemove', () => {
        lastMouseMoveTime = Date.now();
    });

    // Modify the sendFarm function to include the mouse movement check
    const sendFarm = function ($this) {
        const currentTime = Date.now();
        const timeSinceLastMove = (currentTime - lastMouseMoveTime) / 1000; // Convert to seconds

        // Check if the mouse has moved within the last 60 seconds
        if (timeSinceLastMove > 60) {
            UI.ErrorMessage('Are you still there? Please move your mouse to continue.');
            return;
        }
